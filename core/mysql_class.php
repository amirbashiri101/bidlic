...
class MysqliDb
{
    ...
    public function redirect($loc,$type=null,$text=null)
    {
        if($type != ''){
            setcookie('notif', json_encode(['type' => $type, 'text' => $text]), time() + 5, "/");
        }
        if( ! headers_sent() ) {
            header('Location: '.$loc);
        }
    ...
    public function deletecoki($name){
        setcookie($name,false);
        unset($_COOKIE[$name]);
    }
    public function is_admin($id){
        $cnt = $this->where('id',$id)->getOne('users','count(*) as cnt');
        return $cnt['cnt'];
    }
    ...
    public function check_hashSSHA($password, $salt) {
        ...
        if(is_file($im_filename))
        {
            ...
            if(!is_file($th_filename))
            {
                ...
                if($thumb_width > $max_width)
                {
                    ...
                    $thumb_height   = round($thumb_width);
                }
                $width = $thumb_width;
                $height = $thumb_height;
    ...
    public function pagination($item_count, $limit, $cur_page, $link)
    {
        ...
        $first_page = $cur_page > 3 ? '<li class="paginate_button previous"><a href="'.sprintf($link, '1').'">1</a></li>'.($cur_page < 5 ? '' : ' <li class="paginate_button previous disabled"><a href="#">...</a></li> ') : null;
        $last_page = $cur_page < $page_count-2 ? ($cur_page > $page_count-4 ? '' : ' <li class="paginate_button previous"><a href="#">...</a></li> ').'<li class="paginate_button previous"><a href="'.sprintf($link, $page_count).'">'.$page_count.'</a></li>' : null;

        $previous_page = $cur_page > 1 ? '<li class="paginate_button previous"><a href="'.sprintf($link, ($cur_page-1)).'">قبلی</a></li> ' : null;
        $next_page = $cur_page < $page_count ? ' <li class="paginate_button previous"><a href="'.sprintf($link, ($cur_page+1)).'">بعدی</a></li>' : null;

        for ($x=$current_range[0];$x <= $current_range[1]; ++$x)
            $pages[] = '<li class="paginate_button previous"><a href="'.sprintf($link, $x).'">'.($x == $cur_page ? '<strong>'.$x.'</strong>' : $x).'</a></li>';
